...
public class BytecodeCreator implements Visitor {
	...
	private void emitStandardConstructor(String baseClass) {
		...
		Bytecode.code("invokenonvirtual " + baseClass + "/<init>()V");
	...
	public BytecodeCreator start(Program root) {
		visit(root);
		return this;
	}
	...
	public void visit(ClassDeclSimple cd) {
		ClassDeclExtends cde = new ClassDeclExtends(cd.classId, new Identifier(
				"java/lang/Object", null), cd.fieldVarList, cd.methodList);
	...
	public void visit(Formal n) {
		throw new IllegalArgumentException("BytecodeCreator::Formal");
	}
	...
	public void visit(Print printInt) {
		...
		Bytecode.code("invokestatic java/lang/String/valueOf(I)Ljava/lang/String;");

		Bytecode.code("invokevirtual java/io/PrintStream/println(Ljava/lang/String;)V");
		decrStack(2);
	}
	...
	public void visit(Identifier id) {
		...
		if (!isLocalVar(varN)) {
			...
			Bytecode.code(withConstant("aload", 0));
			incrStack();
			Bytecode.code("getfield " + currentClass + "/" + varN + " "
					+ varTypeD);
		} else {
			MethodDescriptor md = getMethodDescriptor();
	...
	public void visit(PrintString printStr) {
		...
		incrStack(2);
		Bytecode.code("invokevirtual java/io/PrintStream/print(Ljava/lang/String;)V");
		decrStack(2);
	}
